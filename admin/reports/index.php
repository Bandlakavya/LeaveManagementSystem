            ...
            <div class="col-2 row align-items-end pb-1">
                <div class="w-100">
                    <div class="form-group d-flex justify-content-between align-middle">
                        <button style="margin-right: 10px;" class="btn btn-flat btn-default bg-lightblue" type="button" id="filter"><i class="fa fa-filter"></i> Filter</button>
                        <br>
                        <button style="margin-right: 10px;" class="btn btn-flat btn-success" type="button" id="export-excel"><i class="fa fa-print"></i> Export PDF</button>
                        <br>
                        <button style="margin-right: 10px;" id="download-excel" class="btn btn-success"><i class="fa fa-file-excel"></i>Download Excel</button>
                    </div>
                </div>
            </div>
        </div>
					...
					?>
						<tr>
							...
							<td>
                                ...
                                <small><b>Name: </b><?php echo $row['firstname'] ?></small>
                            </td>
...
<script>
	$(document).ready(function(){
        ...
        start_loader()
        var _el = $('#print_out').clone();
        _el.find('thead th:last-child, tbody td:last-child').remove();
        _el.prepend('<tr><th colspan="6" style="text-align: center; font-size: 24px;">Leave Application Reports</th></tr>');
        _el.prepend('<tr><th colspan="6" style="text-align: center; font-size: 18px;"></th></tr>');
        _el.prepend('<tr><th colspan="6" style="text-align: center; font-size: 14px;">Generated Date: '+new Date().toLocaleDateString()+'</th></tr>');
        _el.prepend('<tr><th colspan="6" style="text-align: center; font-size: 14px;">Generated Time: '+new Date().toLocaleTimeString()+'</th></tr>');
        _el.prepend('<tr><th colspan="6" style="text-align: center; font-size: 14px;">Total Leave Applications: '+$('#print_out tbody tr').length+'</th></tr>');
        var uri = 'data:application/vnd.ms-excel;base64,'
        , template = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table>{table}</table></body></html>'
        , base64 = function (s) { return window.btoa(unescape(encodeURIComponent(s))) }
        , format = function (s, c) { return s.replace(/{(\w+)}/g, function (m, p) { return c[p]; }) }
        let _elHTML = _el.html()
        _elHTML = '<table>' + _elHTML + '</table>';
        let link = document.createElement("a");
        link.download = "Leave_Application_Reports.xls";
        link.href = uri + base64(format(template, { worksheet: 'Sheet1', table: _elHTML }));
        link.click();
        end_loader()
    })
	})
